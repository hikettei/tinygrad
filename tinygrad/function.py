...
class Sin(Function):
  def forward(self, x: LazyBuffer, fast:bool=False) -> LazyBuffer:
    ...
    self.fast_approx = x.dtype in [dtypes.bfloat16, dtypes.float16, dtypes.float32, dtypes.float64]
    if self.fast_approx:
...
class Log(Function):
  def forward(self, x:LazyBuffer) -> LazyBuffer:
    self.x = x
    fast_approx = x.dtype in [dtypes.bfloat16, dtypes.float16, dtypes.float32, dtypes.float64]
