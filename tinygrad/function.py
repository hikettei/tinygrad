...
def _upperf(x:LazyBuffer) -> LazyBuffer:
  ...
  m = x.cast(cast_to)
  mask = {
    dtypes.float64: 0xfffffffff8000000,
    dtypes.float32: 0xfffff000
  }[x.dtype]
  return m.e(BinaryOps.AND, m.const(mask)).cast(x.dtype)
...
def _pow2if(q: LazyBuffer) -> LazyBuffer:
...
class Exp(Function):
  def forward(self, x:LazyBuffer) -> LazyBuffer:
    fast_approx = False#x.dtype in [dtypes.float32, dtypes.float64]
