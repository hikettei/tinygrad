...
.visible .entry"""
  ...
  asm_for_op: Dict[Op, Callable] = {
    ...
    BinaryOps.AND: lambda d, a, b, dt, name: 'and' + f"{'.lo' if dtypes.is_int(dt) else ''}.{name} {d}, {a}, {b};",#f"and.lo.pred {d}, {a}, {b};" if name == "pred" else f"and.lo.{name} {d}, {a}, {b};",
    BinaryOps.OR: lambda d, a, b, dt, name: 'or' + f"{'.lo' if dtypes.is_int(dt) else ''}.{name} {d}, {a}, {b};",#BinaryOps.OR: lambda d, a, b, dt, name: f"or.pred {d}, {a}, {b};" if name == "pred" else f"or.{name} {d}, {a}, {b};",
