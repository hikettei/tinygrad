...
from tinygrad.codegen.winograd import WinogradMeta, WINOGRAD_TAG
...
class Tensor(MathTrait):
  ...
  def conv2d(self, weight:Tensor, bias:Tensor|None=None, groups=1, stride=1, dilation=1, padding:int|tuple[int, ...]=0,
    ...
    stride_tuple = make_tuple(stride, len(HW))
    dilation_tuple = make_tuple(dilation, len(HW))
    ...
    w_view = weight.reshape(1, groups, rcout, *[1] * len(oyx), cin, *HW)
    mul = x * w_view
    sum_axes = [-1-i for i in range(1+len(oyx))]
    sum_dtype = to_dtype(dtype) if dtype is not None else None
    summed = mul.sum(sum_axes, keepdim=True, dtype=sum_dtype)

    winograd_candidate = (len(HW) == 2 and groups == 1 and
      all(k == 3 for k in HW) and all(s == 1 for s in stride_tuple) and all(d == 1 for d in dilation_tuple))
    if winograd_candidate:
      meta = WinogradMeta(
        input_uop=self.uop,
        weight_uop=weight.uop,
        bias_uop=None if bias is None else bias.uop,
        groups=groups,
        stride=stride_tuple,
        dilation=dilation_tuple,
        padding=tuple(map(int, padding_)),
        dtype=sum_dtype,
        output_shape=(bs, cout, *oyx),
        input_shape=self.shape,
        weight_shape=weight.shape,
      )
      summed.uop = summed.uop.replace(tag=(WINOGRAD_TAG, meta))

    ret = summed.reshape(bs, cout, *oyx)
    if bias is not None: ret = ret.add(bias.reshape(1, -1, *[1] * len(HW)))
    return ret
